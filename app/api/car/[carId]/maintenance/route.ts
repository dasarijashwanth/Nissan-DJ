...
import { isCarOwnedBy, getMaintenanceLogs, createCarTransaction } from "@/lib/carQueries";
import { validateMaintenanceLog, type MaintenanceFormValues } from "@/lib/carValidation";
...
export async function GET(_request: Request, { params }: { params: Promise<{ carId: string }> }) {
  ...
  return NextResponse.json(await getMaintenanceLogs(carId));
}
...
export async function POST(request: Request, { params }: { params: Promise<{ carId: string }> }) {
  ...
  await createCarTransaction(user.id, "maintenance", values.type, cost, date);
