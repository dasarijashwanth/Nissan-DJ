...
export function ChitFundByGroupChart({ data }: { data: { groupName: string; amount: number }[] }) {
  ...
  return (
    <Card className="p-5">
      <p className="mb-4 text-sm font-medium text-text-secondary">By Chit Group</p>
      ...
      <ul className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs text-text-secondary">
        {data.map((d, i) => (
          <li key={d.groupName} className="flex items-center gap-1.5 truncate">
