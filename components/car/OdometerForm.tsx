...
export function OdometerForm({ open, onClose, carId, currentOdometer }: OdometerFormProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const res = await fetch(`/api/car/${carId}/odometer`, {
        method: "POST",
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {milesSinceLast > 0 && (
          <p className="rounded-lg bg-amber-50 px-3 py-2 text-sm font-medium text-amber-700">
            {milesSinceLast.toLocaleString()} miles since your last log
          </p>
        )}
        <Textarea
