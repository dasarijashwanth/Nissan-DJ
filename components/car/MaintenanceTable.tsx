...
import { isMaintenanceDueSoon } from "@/lib/carUtils";
    ...
    return (
      <>
        <Card className="flex flex-col items-center justify-center gap-3 py-16 text-center">
          ...
          <p className="text-sm font-medium text-slate-700">No maintenance logged yet</p>
        ...
        <MaintenanceForm key={formOpen ? "open" : "closed"} open={formOpen} onClose={() => setFormOpen(false)} carId={carId} />
      </>
    );
  }
                ...
                <tr key={l.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-500">
                    {l.nextDueDate
                      ...
                      : l.nextDueMiles
                        ? `${l.nextDueMiles.toLocaleString()} mi`
                        : "—"}
                  </td>
                </tr>
              ))}
